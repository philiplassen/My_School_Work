...
count = model.evaluate(x = x_test, y = y_test)
print("cumulative loss = " + str(count))
print(model.metrics_names)
"""
f = np.array(color.rgb2gray(io.imread('test_images/image/1039_3_image.png')))
...
def dice(f, g, c):
  return 2 * np.sum(np.logical_and((f == c), (g == c))) / (np.sum(f == c) + np.sum(g == c))
...
segmented = pred(f)
k = np.array(color.rgb2gray(io.imread('test_images/seg/1039_3_seg.png')))
dic(segmented, k)
...
ax[1].imshow(k, cmap = "gray")
ax[1].set_title("Ground Truth Image")
ax[2].imshow(k == segmented)
ax[2].set_title("Classification Match")
